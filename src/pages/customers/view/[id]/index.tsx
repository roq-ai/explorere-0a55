...
function CustomerViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<CustomerInterface>(
    ...
    () =>
      getCustomerById(id, {
        relations: ['user', 'sales_representative', 'organization'],
      }),
  );
              ...
              >
                ...
                {hasAccess('sales_representative', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    ...
                    text={
                      <Link as={NextLink} href={`/sales-representatives/view/${data?.sales_representative?.id}`}>
                        {data?.sales_representative?.id}
                      </Link>
                    }
                  />
                )}
