...
import { getSalesRepresentatives } from 'apiSdk/sales-representatives';
...
import { SalesManagerListPage as ShowCaseList } from 'pages/sales-managers';
...
function DashboardPage() {
  ...
  const salesRepresentativeStat = useSWR<PaginatedInterface<SalesRepresentativeInterface>>(
    () => `/sales-representatives`,
    () => getSalesRepresentatives({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    },
    {
      ...
      result: salesRepresentativeStat?.data?.totalCount,
      isLoading: salesRepresentativeStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Guests',
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Sales Managers
                </Text>
                <NextLink href={`sales-managers`} passHref legacyBehavior>
                  <Button
